...
const ExpenseDashboard = () => {
  ...
  const queryRange = useMemo(() => getDateRange(range), [range]);
  const { from, end } = queryRange;
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      {isLoading || isFetching ? (
        ...
        <Card
          className="rounded-2xl border border-red-200 bg-card shadow-sm"
          sx={{ mb: 3 }}
        >
          <CardContent>
            <Typography color="error">
              Failed to load expenses: {JSON.stringify(error)}
            </Typography>
          </CardContent>
        </Card>
      ) : (
